  ...
  <div id="black_overlay" style="width: 100%; visibility:hidden;"> 
    <div class="added pop-up">
      ...
      <form name="situation" id="situation" action="" method="post" enctype="multipart/form-data">
		...
		<span class="star">+</span>Upload Photo
        <input type="file" name="request_media[]" id="files" class="input-file" multiple="multiple">  
        
		</div>
		<div class="gallery" id="list"></div>
	
		<div class="submit-media">
        <input type="submit" class="send_request" name="submit_situation" id="submit_situation"value="Send Request"/>
		</div>
      </form>      
    ...
    <div class="row enigma_blog_wrapper">
      ...
      <div class="form-wrap">
          ...
          <label>Location: </label>
           ...
           <input name="search_loc" id="search_loc" type="text" value="<?php echo $_POST['search_loc'] ; ?>" placeholder="i.e. Seattle, WA" />
        ...
        <form class="all_jobs" name="search_form" action="" method="post">
          ...
          <label>Author: </label>
           ...
           <input name="search_auth" id="search_auth" type="text"  placeholder="i.e. John Smith" value="<?php echo $_POST['search_auth'] ; ?>" />
...
else{ $args1 = array('post_type'=>'task','posts_per_page'=>-1,'paged' => $paged,'author'=>$author_id ,'meta_query'=>
 array(array(
        'key'       =>'final_release',
        'value'     => 'no',
        'compare'   => 'LIKE'
...
else if(isset($_POST['search_post1']))
{
function get_user_id_by_display_name_cat( $display_name) {
global $wpdb;
if ( ! $user = $wpdb->get_row( $wpdb->prepare(
"SELECT `ID` FROM $wpdb->users WHERE `display_name` = %s", $display_name
)))
...
$author_name = $_POST['search_auth'];
$author_id = get_user_id_by_display_name_cat($author_name);
//echo $author_id;die;
if($author_id ==''){$args = array('post_type'=>'task','posts_per_page'=>9,'author'=>9.3);}
else{ $args = array('post_type'=>'task','posts_per_page'=>-1,'author'=>$author_id ,'meta_query'=>
 array(array(
        'key'       =>'final_release',
        'value'     => 'no',
        'compare'   => 'LIKE'
...
jQuery(document).ready(function()
{
	jQuery(".request_click").click(function(){
		var h=jQuery("body").height()+'<span id="IL_AD12" class="IL_AD">px</span>';jQuery("#black_overlay").css({"height":h,"visibility":"visible"});jQuery(".added").css('display','block');jQuery('.request_amount').text('$'+jQuery(this).attr('data-amt'));jQuery('.request_author').text(jQuery(this).attr('data-authname'));jQuery('.author_of_post').val(jQuery(this).attr('data-authid'));jQuery('.request_for_post').val(jQuery(this).attr('data-jobid'))});jQuery(".close").click(function(){jQuery(".added").css('display','none');jQuery("#black_overlay").css("visibility","hidden")});jQuery('.request_click1').click(function(){alert('Please check your email to activate your WePay account')});
...
{$("#"+id+" .load-anchor-cate").hide();$(div_id).before(posts)}
else{var div_id="#"+id+" .load-more-post-cate";$(div_id).before(posts)}$(".loader").fadeOut("slow");})})});jQuery(document).ready(function($){var ajaxUrl="<?php echo admin_url('admin-ajax.php')?>";var page=1;var ppp=9;var x=0;var y=0;$(".cate-small-business").on("click",function(){$(".loader").show();var id=$(this).attr('id');$.post(ajaxUrl,{type:'POST',action:"load_more_post",data:{'data-id':id},offset:(page*ppp),ppp:ppp}).success(function(posts){page++;y=y+450;var div_id="#"+id+" .load-more-post-cate";if(posts=='<div class="col-md-12 no-more-posts"> No More Posts Available. </div>')
{$("#"+id+" .load-anchor-cate").hide();$(div_id).before(posts)}
