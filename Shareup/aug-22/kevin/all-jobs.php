...
<?php $wcatTerms = get_terms('types', array('hide_empty' => 0, 'parent' =>0)); 
  ...
  <li>
      ...
      <a data-toggle="tab" href="#<?php echo $wcatTerm->slug; ?>"><?php echo $wcatTerm->name; ?></a>
...
<?php 
  ...
  <label>Location: </label>
   ...
   <input name="search_loc" id="search_loc" type="text" placeholder="i.e. Anchorage, AK" />

   <input type="submit" name="search_job" id="search_job" style="visibility: hidden;" />
...
<form class="all_jobs" name="search_form" action="" method="post">
  ...
  <label>Author: </label>
   ...
   <input name="search_auth" id="search_auth" type="text"  placeholder="i.e. Sapna Rana" />
...
if($author_id ==''){
$args1 = array('post_type'=>'task','posts_per_page'=>9,'paged' => $paged,'author'=>9.3);
 }
 else{
 $args1 = array('post_type'=>'task','posts_per_page'=>-1,'paged' => $paged,'author'=>$author_id);
...
$args11 = array('post_type'=>'task','posts_per_page'=> 9,'paged' => $paged,'post_status'=>'publish');  
...
{
 $count=count($app_arr1);
  echo '<a class="request_click1">Request-hahah</a>';
   echo "<br>";
 echo '<div class="req-color">Requests '.$count .'</div>' ;
...
{
	  echo '<div class="req-color">Requested </div>' ;
 }
...
{
$count=count($app_arr1);
echo '<a href="'.get_bloginfo('url').'/log-in">Login for Request</a>';
echo "<br>";
echo '<div class="req-color">Request '.$count .'</div>' ;
...
jQuery('.request_click1').click(function () {

alert('Please verify your wepay account');

})

         jQuery("#search_loc").autocomplete({
	...
	/* pagination code */
	  ...
	  var imgUrl = jQuery('#gif').val();
	  jQuery("html, body").animate({ scrollTop: 0 }, 600);
	  jQuery('#all').html("<img src='" + imgUrl + "' alt='description' style='margin:50px 0px 0px 400px'/>");	 
	  jQuery('#all').load(link + ' #all');
...
jQuery(document).ready(function($){
	...
	// category pagonation family-youth
	   jQuery(document).ready(function($){
		var ajaxUrl = "<?php echo admin_url('admin-ajax.php')?>";
		var page = 1; 
		var ppp = 9; 
	...
	// category pagonation food-basics
	   jQuery(document).ready(function($){
		var ajaxUrl = "<?php echo admin_url('admin-ajax.php')?>";
		var page = 1;
		var ppp = 9; 
		var x=0; var y=0;
	   $(".cate-food-basics").on("click",function(){
		   var id = $(this).attr('id');
	...
	// category pagonation home-rent
	   ...
	   jQuery(".cate-home-rent").on("click",function(){
		...
		// category pagonation medical
	   jQuery(document).ready(function($){
		var ajaxUrl = "<?php echo admin_url('admin-ajax.php')?>";
		var page = 1; 
		var ppp = 9; 
		var x=0; var y=0;
	   $(".cate-medical").on("click",function(){
		   var id = $(this).attr('id');
		   ...
		   $.post(ajaxUrl, {
			...
			var div_id = "#" + id + " .load-more-post-cate"; // #home-rent .load-more-post-cate
				$(div_id).before(posts); 
			}
		...
		// category pagonation small-buisness
	   jQuery(document).ready(function($){
		var ajaxUrl = "<?php echo admin_url('admin-ajax.php')?>";
		var page = 1; 
		var ppp = 9; 
		var x=0; var y=0;
	   $(".cate-small-buisness").on("click",function(){
		   var id = $(this).attr('id');
		   $.post(ajaxUrl, {
			   type:'POST',
            action:"load_more_post",
   ...
   // category pagonation unemployment
	   jQuery(document).ready(function($){
		var ajaxUrl = "<?php echo admin_url('admin-ajax.php')?>";
		var page = 1;
		var ppp = 9; 
		var x=0; var y=0;
	   $(".cate-unemployment").on("click",function(){
		   var id = $(this).attr('id');
...
.load-anchor {text-align: center;}
.load-anchor-cate{text-align: center;}
#more_posts {
font-size: 14px;
color: #8cc53e;
font-weight: bold;
